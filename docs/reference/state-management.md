...
All three context providers are instantiated in `src/main.tsx` and wrap the entire React tree. They are never instantiated inside a screen or component.
...
**Nesting order matters:** `StoreProvider` reads from both `useSyncStore()` and `useSettingsStore()`, so it must be innermost. `SyncProvider` reads from `SettingsService` (not from `useSettingsStore`), so it can sit between the other two.
...
Manages the full checklist state. The implementation is decomposed across several files for maintainability — see `docs/snapshots/store-catalog.md` for the full module map.
...
- `selectedGroupID: string | null` — the active group filter; `null` means "All". Initialized to the first group's ID if groups exist on load, otherwise `null`. **Not separately persisted** — derived from the groups array on load.
...
This derived value is computed in `useMemo` inside `useCategoryDerived`:

- When `selectedGroupID === null` ("All"): returns all `state.categories`.
- When a specific group is selected: returns `categories.filter(c => c.groupID === selectedGroupID)`.

Categories with `groupID === undefined` (ungrouped) are **not** included when a specific group is active — they only appear in the "All" view.
...
2. A `useEffect` in `useCategoryDerived` detects if the current `selectedCategoryID` is not in the newly computed `categoriesInSelectedGroup`.
3. If the selection is not visible, it auto-selects the first category in the group.
4. `selectedGroupID` is **not separately persisted** — it is initialized from the groups array on load.
...
| `REORDER_CATEGORIES`          | `orderedIDs: string[]`                         | Reorders categories to match an array of ordered IDs                                                                                                                                                           |
...
| `RELOAD`                      | —                                              | Re-reads `localStorage` via `PersistenceService.load()` and replaces all state; does **not** re-save; preserves `selectedGroupID` if the group still exists, otherwise falls back to the first group or `null` |
| `RESET_CATEGORIES`            | —                                              | Clears all categories **and groups**; sets `selectedCategoryID = ""`; writes empty state to `localStorage`                                                                                                     |
| `SYNC_LOAD`                   | `categories`, `selectedCategoryID?`, `groups?` | Replaces all data from a cloud sync payload; preserves `selectedGroupID` if the group still exists; persists to `localStorage`                                                                                 |
...
| `ADD_CATEGORY_WITH_GROUP`     | `name`, `groupID`                              | Atomically creates a new category and assigns it to the given group                                                                                                                                            |
...
| `reorderCategories(orderedIDs)`          | `(orderedIDs: string[]) => void` | Reorders categories to match an array of IDs                                                                |
...
| `addCategoryWithGroup(name, groupID)`    | —                                | Dispatches `ADD_CATEGORY_WITH_GROUP`; atomically creates and assigns                                        |
